        ...
        <ul className="mt-4 grid gap-2">
          {speakers.map((speaker) => (
            <li key={speaker.id}>
              <button
                type="button"
                className="flex w-full items-center gap-3 rounded-[8px] border border-[var(--cyc-line)] bg-white p-2 text-left dark:border-white/10 dark:bg-[var(--cyc-navy)]"
                onClick={() => {
                  void activeApp.callServerTool({
                    name: 'get_speaker',
                    arguments: { id: speaker.id },
                  });
                }}
              >
                <SpeakerPhoto
