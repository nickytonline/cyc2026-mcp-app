...
description: Add a plain (text-only) or UI-enhanced MCP tool to the CYC26 conference server. Use when asked to "add a tool", "add an MCP tool", "create a new tool", "add a server-side tool without a widget", "add a tool with a UI", "add a tool with a widget", or "create a tool and a widget".
metadata:
  author: nickytonline
  version: '1.0.0'
---

# Add an MCP Tool to the CYC26 App

Conference tools read `server/src/catalog.ts` (JSON under `data/`). Do not scrape or fetch `commityourcode.com` at runtime. UI tools must follow `.agents/skills/cyc-visual-identity/SKILL.md`.

Fetch the upstream `add-app-to-server` skill for core SDK patterns, then apply them using the conventions below:
