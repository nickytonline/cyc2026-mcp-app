...
This is the **Commit Your Code 2026 (CYC26)** MCP app: a conference companion for speakers, talks, schedule, and side events. It is built on the Pomerium MCP Apps TypeScript template.

- **MCP Server** (Node.js + Express): Registers conference tools and serves widget HTML
- **JSON catalog** (`data/*.json`): Speakers, sessions, schedule, events, and tracks. There is no public CYC API — tools read committed JSON via `server/src/catalog.ts`. Never scrape or fetch `commityourcode.com` at runtime.
- **MCP App Views**: React widgets that match the [CYC website](https://www.commityourcode.com/) look and feel
...
| `get_schedule`       | `ui://schedule`       | Required `day` 0/1/2. Optional `track` / `room`. Day picker updates the current view. Test in MCPJam with `day=1` |
...
- Widget name comes from the filename (e.g., `speakers-list.tsx` → `speakers-list` widget)
- **Widgets must include their own mounting code** at the bottom of the file
- Generates content-hashed assets (e.g., `speakers-list-a1b2c3d4.js`)
...
widgets/src/
  ...
  │   ├── speakers-list.tsx
  │   └── session-detail.tsx
  ├── speakers-list/              # Widget-specific components
  │   ├── SpeakersList.tsx
  │   └── SpeakersList.stories.tsx
  ├── components/                 # Shared components (including shadcn/ui)
  │   ├── cyc/                    # CYC chrome: WidgetShell, TrackChip
  │   └── ui/
  ├── hooks/                      # Shared hooks
...
- `server/src/catalog.ts` - Reads `data/*.json` and implements filters/search
- `server/src/types.ts` - Zod schemas and TypeScript interfaces
- `data/*.json` - Committed CYC26 catalog (speakers, sessions, schedule, events, tracks)
- `server/tests/*.test.ts` - Vitest specs for tools, catalog, and stateless transport behavior
...
- Files include both hashed versions (`speakers-list-{hash}.js`) and unhashed (`speakers-list.js`)
...
- Keep React components in PascalCase modules (e.g. `SpeakersList.tsx`)
...
4. Add the app to a chat and test with: `list the CYC speakers on the AI track`
...
## Refreshing conference data

There is no public CYC API. Re-run the one-shot ingest when the website lineup changes:

```bash
node scripts/ingest-cyc.mjs
```

Commit the updated files under `data/`. Tools must keep reading JSON — do not scrape at request time.
